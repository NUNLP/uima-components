...
class TestGroovyDSL {
	...
	@Test
	public void testSelectEmptyResult() {
		jcas.setDocumentText(
				"""\
        There are no entity mentions in this text.
        """
				);
		SimplePipeline.runPipeline(this.jcas, this.engine)

		Collection<UMLSRelation> anns = select(type:UMLSRelation)
		assert anns.size() == 0
		
		CAS cas = jcas.getCas()
		Type type = JCasUtil.getType(jcas, UMLSRelation)
		FSIterator<FeatureStructure> fsiter = cas.getIndexRepository().getAllIndexedFS(type)
		FSIterator<FeatureStructure> copy = fsiter.copy()
	}
	...
	public void testCreate() {
		...
		ontologyConcepts:[create(type:UmlsConcept, codingScheme:"SNOMED", code:1, cui:"C01", tui:"T01"), create(type:UmlsConcept, codingScheme:"SNOMED", code:2, cui:"C02", tui:"T02")
		])
