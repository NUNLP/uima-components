...
public class AhoCorasickDict {
    ...
    AhoCorasickDict() {
        ...
        this.root.addChild(FAILLINK, root)
    }

    public AhoCorasickDict(String dictfile) {
        this.root = new Node(null)
        this.root.addChild(FAILLINK, root)
        log.info "building aho-corasick trie from: $dictfile"
        try {
            ...
            reader.close()
            faillinking()
            log.info "built aho-corasick trie. $count terms"
        ...
        if (child == null) {
            ...
            while (n != root) {
                if (n.phrase != null) {
                    ...
                    if (tags.get(i - len) == null)
                        currentTags = new Vector<String>()
                    else
                        currentTags = tags.get(i - len)
                    currentTags.add(tag)
                    tags.set(i - len, currentTags)
                }
    ...
    void faillinking() {
        ...
        while (ll.size() > 0) {
            ...
            while (it.hasNext()) {
                ...
                while (fail != root && fail.getChild(key) == null)
                    fail = fail.getChild(FAILLINK)
                if (fail.getChild(key) != null)
                    node.getChild(key).addChild(FAILLINK, fail.getChild(key))
            }
        }
    }
}
